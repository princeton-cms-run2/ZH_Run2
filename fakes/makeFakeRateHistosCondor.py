...
for line in open(args.inFileName,'r').readlines() :
	...
	#for period in ['SingleMuon', 'EGamma', 'MuonEG'] :
	    for dataset in ['data'] :
		inFileRoot = 'data_{0:s}.root'.format(era)
		...
		for i, e in enumerate(inTree) :
		    ...
		    if args.region == 'SS'and e.nbtag[0] > 0 : continue
		    if args.region == 'OS'and e.nbtag[0] < 1 : continue
		    
		    #if abs(e.dZ_3) > 0.05 or abs(e.dZ_4) > 0.05 : continue
		    if cat[:2] == 'mm' and  (e.iso_1 > iso_mu or e.iso_2 > iso_mu) : continue
		    if cat[:2] == 'mm' :
			if  (e.isGlobal_1 < 1 and e.isTracker_1 < 1) or e.tightId_1 < 1 : continue
			if  (e.isGlobal_2 < 1 and e.isTracker_2 < 1) or e.tightId_2 < 1 : continue
			...
			for wp in WP : 
			    ...
			    if int(e.idDeepTau2017v2p1VSjet_4) >= int(wp)-1 and int(e.idDeepTau2017v2p1VSe_4)>=int(tau_vse-1) and int(e.idDeepTau2017v2p1VSmu_4)>=int(tau_vsmu-1) :
			...
			if preCutOff or preCutt :hBase['m_mt'].Fill(e.pt_3)
			hBase['t_mt'].Fill(e.pt_4)
			if e.decayMode_4 != -1 : 
			    hBaseMode['data']['t_mt'][str(e.decayMode_4)].Fill(e.pt_4)

			for wp in WP : 
			    if e.iso_3 < iso_mu and  (e.isGlobal_3 > 0 or e.isTracker_3 > 0) and e.tightId_3 > 0: 
			...
			for wp in WP : 
			    if int(e.idDeepTau2017v2p1VSjet_3) >= int(wp)-1  and int(e.idDeepTau2017v2p1VSe_3)>=int(tau_vse-1) and int(e.idDeepTau2017v2p1VSmu_3)>=int(tau_vsmu-1) :     
			...
			if preCutOff or preCutt :hBase['m_em'].Fill(e.pt_4)

			for wp in WP : 
			    if e.iso_3 < 0.15 and  e.Electron_mvaFall17V2noIso_WP90_3 > 0 : hTight['e_em'][wp].Fill(e.pt_3)
			    if e.iso_4 < iso_mu and  (e.isGlobal_4 > 0 or e.isTracker_4 > 0) and e.tightId_4 > 0: hTight['m_em'][wp].Fill(e.pt_4)
...
DYJetsW=1.4628040e+08
WJetsW=1.1542430e+08

if not DYIncl_only : 
    for i in range(1,5) :
	nn = 'DY{0:d}JetsToLL'.format(i)
	if search(nickNames, nn) :
	    sampleWeight[nn] = Pblumi*weights['lumi']/(DYJetsW/6225.42 + DYxGenweightsArr[i-1]/(xsec[nn]*DY_kfactor))
...
if not WIncl_only:
	...
	if search(nickNames, nn) : 
	    if search(nickNames, 'WJetsToLNu') : sampleWeight[nn] = Pblumi*weights['lumi']/(WJetsW/61526.7 + WNJetsXsecs[i-1]/(xsec[nn]*WJets_kfactor))
	...
	for i, e in enumerate(inTree) :
	   ...
	   #recoils 
	    ...
	    if 'data' not in group : 
                weight = e.Generator_weight *sWeight *e.weightPUtrue 
                weight *= e.L1PreFiringWeight_Nom
		...
		if e.gen_match_3 == 15 :
		    ...
		    if preCutOff or preCutt : hBasePrompt[group]['m_mt'].Fill(e.pt_3,weight)
		...
		if e.gen_match_3 != 15 and e.gen_match_3 != -1 :
		    ...
		    if preCutOff or preCutt  : hBasenoPrompt[group]['m_mt'].Fill(e.pt_3,weight)
		...
		if e.gen_match_4 == 15 :
		    ...
		    if preCutOff or preCutt : hBasePrompt[group]['m_em'].Fill(e.pt_4,weight)
                    for wp in WP : 
		        if e.iso_4 < iso_mu and  (e.isGlobal_4 > 0 or e.isTracker_4 > 0) and e.tightId_4 > 0: hTightPrompt[group]['m_em'][wp].Fill(e.pt_4,weight)
		...
		if not(e.gen_match_4 != 0 and e.gen_match_4 != 3 and e.gen_match_4 != 4 and e.gen_match_4 != 5) and e.gen_match_4 != -1:
		    ...
		    if preCutOff or preCutt : hBasenoPrompt[group]['m_em'].Fill(e.pt_4,weight)
                    for wp in WP : 
		        if  (e.iso_4 < iso_mu and  (e.isGlobal_4 > 0 or e.isTracker_4 > 0) and e.tightId_4 > 0): hTightnoPrompt[group]['m_em'][wp].Fill(e.pt_4,weight)
