...
for sys in sysall : 
    ...
    outFileName='allGroups_{0:s}_OS_LT00_ll_{1:s}_sys{2:s}.root'.format(era, tag, sys)
	...
	for plotVar in histos :
		...
		if sys == 'Central' : plotVar =''
		    ...
		    #h[i].SetTitle(hname)
                    ...
                    if ( 'PTV' in hplus or 'FWDH' in hplus )and 'ZH' not in group  : continue
		    else:     h[i].Write()
