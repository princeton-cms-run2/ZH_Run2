...
def getArgs() :
    ...
    parser.add_argument("--nickName",default='DY1JetsToLL',help="MC sample nickname") 
...
for count, e in enumerate(inTree) :
    ...
    for lepMode in ['ee','mm'] :
        #print("Lepmode = {0:s}".format(lepMode))
        if lepMode == 'ee'  :
        ...
        for tauMode in ['et','mt','tt'] :
            ...
            #if ZHDR(e,LepP,jt1) < 0.5 or ZHDR(e,LepP,jt2) < 0.5 or ZHDR(e,LepM,jt1) < 0.5 or ZHDR(e,LepM,jt2) < 0.5 : continue
            #cutCounter[cat].count('DR') 
