...
def getArgs() :
    ...
    parser.add_argument("-c","--concatenate",default=10,type=int,help="On how many files to run on each job")
...
for nFile in range(0, len(dataset),mjobs) :
    ...
    for j in range(0,maxx) :
        ...
        outLines.append("python ZH.py -f inFile.root -o {0:s} --nickName {1:s} -y {2:s} -l AZH -w 0 -d Data \n".format(outFileName,args.nickName, args.year))
